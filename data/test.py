...
from . import symbols, stock_history, get_stock_history_sample
...
class TestStockPriceHistory(TestCase):
    ...
    def test_stock_history_type(self):
        history = stock_history(self.symbols)
        self.assertIsInstance(history, pd.DataFrame)
    
    def test_get_stock_history_sample_type(self):
        history = get_stock_history_sample(3)
        self.assertIsInstance(history, pd.DataFrame)
